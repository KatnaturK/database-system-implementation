...
void QueryPlan::print(std::ostream& os) {
  os << "Number of selects: " << num_selects << endl;
  os << "Number of joins: " << num_joins << endl;
  if (groupingAtts) {
    os << "GROUPING ON ";
    ...
    // for (NameList* att = groupingAtts; att; att = att->next) {
    //   Attribute att;
    //   makeAttr(att, atts->name, cSchema->FindType(atts->name));
    // }
  }
  ...
  // root->print(os);
  QueryNode* node = root;
  printInOrder(node, os);
}

void QueryPlan::printInOrder(QueryNode* node, std::ostream& os) {
  if (node) {
    switch(node->num_children) {
      ...
      case 1:
      {
        UnaryNode* temp = (UnaryNode*)node;
        printInOrder(temp->child, os);
        node->print(os);
        break;
      }
      case 2:
      {
        BinaryNode* temp = (BinaryNode*)node;
        printInOrder(temp->left, os);
        printInOrder(temp->right, os);
        node->print(os);
        break;
      }
    }
...
WriteNode::WriteNode(FILE* out, QueryNode* c):
  UnaryNode("WRITE OUT", c->outSchema, c, NULL), outFile(out) {}
