 ...
 * API                                                                *
 **********************************************************************/
QueryPlan::QueryPlan(Statistics* st): root(NULL), outName("STDOUT"), stat(st), used(NULL), joinQueryCount(0), selectQueryCount(0) {}
...
void QueryPlan::print(std::ostream& os) const {
  ...
  QueryNode* opNode = root;
  printNodesInOrder(opNode, os);
}

void QueryPlan::printNodesInOrder(QueryNode* opNode, ostream& os) const {
  if (opNode) {
    switch(opNode->childNodeCount) {
      ...
      case 1: {
        ...
        printNodesInOrder(temp->child, os);
        opNode->print(os);
        break;
      }
      ...
      case 2: {
        ...
        printNodesInOrder(temp->left, os);
        printNodesInOrder(temp->right, os);
        opNode->print(os);
        break;
      }
    }
