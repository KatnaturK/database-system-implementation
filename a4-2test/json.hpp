...
/*
 * Modified version of https://github.com/nbsdx/SimpleJSON
 * */
namespace json {
    ...
    // Modified by deepakgm on 2/28/2020
    template<typename T>
    JSON Array(T args[],int size) {
        JSON arr = JSON::Make(JSON::Class::Array);
        for (int i = 0; i < size; ++i){arr.append(args[i]);}
        return std::move(arr);
    }
    //
